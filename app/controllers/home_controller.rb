      ...
      when "Admin"
        ...
        @houses = House.all
			...
			@houses.each_with_index do |house, index|
				@tasks_to_approve << Task.joins(:my_tasks).where(my_tasks: {user_id: house.kids.ids}).where(my_tasks: {progress: 1}).ids
				@tasks_to_approve[index].each do |task|
					@kids_with_unapproved_tasks << [task, house.kids.joins(:my_tasks).where(my_tasks: {progress: 1, task_id: task}).ids]
				end
			end
